...
export async function getUser(username) {
    ...
    const lives = await collection.find({ username }, {projection: {_id: 0}}).toArray();

    collection = await db.collection("users");
    await collection.updateOne({ username }, { $set: { lives: lives } });
    const followers = await collection.find({ following: username }, {projection: {username: 1, _id: 0}}).toArray();
